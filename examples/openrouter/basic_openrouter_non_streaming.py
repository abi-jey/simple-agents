"""
Example demonstrating OpenRouter non-streaming usage.

This example shows how to use the Agent with ProviderType.OPENROUTER
in non-streaming mode, which yields TextDoneEvent instead of TextChunkEvents.
"""
...
async def main() -> None:
    """Main example demonstrating OpenRouter non-streaming mode."""
    console.print(Panel.fit("[bold blue]nagents OpenRouter Non-Streaming Example[/bold blue]"))
    ...
    console.print("[dim]Using OpenRouter provider (non-streaming)[/dim]")
    ...
    session_id = f"session-openrouter-nonstream-{datetime.now(UTC).strftime('%Y%m%d-%H%M%S')}"
